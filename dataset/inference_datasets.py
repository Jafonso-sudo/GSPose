...
class LINEMOD_Dataset_GEN6D(torch.utils.data.Dataset):
    def __init__(self, data_root, obj_name, 
        ...
        # Hacky adjustment to allow the GS-Pose dataset architecture to work with
        self.benchmark2objname = {
            "mustard0" : "mustard_bottle",
        }
        assert obj_name in self.benchmark2objname.keys(), f"{obj_name} is not in the benchmark2objname"
        self.benchmark_name = obj_name
        obj_name = self.benchmark2objname[obj_name]
        # ---
        ...
        self.is_symmetric = False # TODO: Check if safe to delete

        self.camK = np.array([
            [572.4114 ,   0.     , 325.2611 ],
            [  0.     , 573.57043, 242.049  ],
            [  0.     ,   0.     ,   1.     ]], dtype=np.float32) # TODO: Add camera intrinsics
        ...
        self.obj_pointcloud = py3d_io.load_ply(self.obj_ply_path)[0].numpy() # TODO: 
...
class YCBInEOAT_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_root, obj_name, 
        ...
        assert(subset_mode in ['train', 'test']), f'{subset_mode} is not a valid subset mode [train, test]'
        assert(obj_name in YCBINEOAT_OBJECTS.keys()), f'{obj_name} is not in the YCBINEOAT {YCBINEOAT_OBJECTS}'
        self.obj_name = obj_name
        self.data_root = data_root
        self.subset_mode = subset_mode
        self.num_grid_points = num_grid_points
        self.benchmark_dir = os.path.join(self.data_root, self.benchmark_name)
        self.obj_dir = os.path.join(self.data_root, 'train', self.obj_name)
        ...
        self.obj_database_dir = obj_database_dir
        self.use_binarized_mask = use_binarized_mask
        self.to_meter_scale = 1e-3 # TODO: Check if YCBINEOAT is in mm or m
...
datasetCallbacks = {
    ...
    'YCBINEOAT': {'OBJECTS': YCBINEOAT_OBJECTS,
                    'DATASETLOADER': YCBInEOAT_Dataset, 
                    'DATAROOT': os.path.join(DATASPACE_DIR, 'ycbineoat'),
                },
}
