...
from inference import create_reference_database_from_RGB_images, ModelParams, PipelineParams, OptimizationParams, create_3D_Gaussian_object, perform_segmentation_and_encoding, multiple_initial_pose_inference, GS_Tracker, render_Gaussian_object_model
...
def load_model_net(ckpt_file: str, device: torch.device = None):
    if not device:
...
def load_test_data(video_directory_path: str):
    ...
    query_video_frames = media.read_video(os.path.join(video_directory_path, 'Frames.m4v')) # NxHxWx3    
    query_video_frames.shape
    
    return query_video_frames, query_video_camKs

def create_or_load_gaussian_splat_from_images(demo_data_dir: str, obj_name: str, model_net: ModelNet, device: torch.device = None):
    if not device:
        device = torch.device('cuda')
...
def perform_pose_estimation(model_net, reference_database, query_video_frames, query_video_camKs, frame_reinit=False, device: torch.device = None):
    if not device:
        device = torch.device('cuda')
...
def render_gaussian_model(gaussian_object: GaussianModel, camK: np.ndarray, R: np.ndarray, T: np.ndarray, H, W, device: torch.device = None):
    ...
    return render_Gaussian_object_model(gaussian_object, camK, pose, H, W, device=device)